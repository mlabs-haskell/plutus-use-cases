...
module Mlabs.PAB.LocalCluster (IsContract, runLocalCluster) where
...
import Plutus.PAB.Effects.Contract.Builtin (HasDefinitions, handleBuiltin)
...
import Servant.API (JSON, MimeUnrender)
...
runLocalCluster :: forall a. IsContract a => IO ()
runLocalCluster = withLocalClusterSetup $ \dir lo@LogOutputs {..} ->
  ...
  where
    ...
    setupFaucet dir trCluster (RunningNode socketPath _ _) = do
      ...
      let trCluster' = contramap MsgCluster trCluster
          encodeAddresses = fmap (first (Text.unpack . encodeAddress @ 'Mainnet))
    ...
    whenReady
      ...
      rn@(RunningNode socketPath block0 (gp, vData)) = do
        ...
        where
          mkUrl :: forall b. ToText b => String -> b -> Text
          mkUrl h p = Text.pack h <> ":" <> toText p
...
withLocalClusterSetup action = do
  putStrLn $
    mconcat
      ...
      , "`local-cluster-data/cardano-node-shelley` "
...
setupPABServices walletHost walletPort dir rn =
  void
    ...
    $ do
      ...
      launchPAB @a fixturePassphrase dir walletUrl rn chainIndexPort
    ...
    let opts =
        ...
        config =
          PAB.Config.defaultConfig
            ...
            , dbConfig =
                def
                  { dbConfigFile = Text.pack $ dir </> "plutus-pab.db"
                  }
            , chainIndexConfig =
                def
                  { PAB.CI.ciBaseUrl =
...
restoreWallets host@(ClusterHost walletHost) port@(ClusterPort walletPort) = do
  ...
  runClient manager >>= \case
    ...
    Right (ApiWallet (ApiT i) _ _ _ _ _ _ _ _) -> do
      ...
      putStrLn $ "Passphrase: " <> Text.unpack fixturePassphrase
      pure baseUrl
  where
    ...
    wpData =
      Wallet.Types.WalletPostData
        ...
        (ApiT . Passphrase . fromString $ Text.unpack fixturePassphrase)
