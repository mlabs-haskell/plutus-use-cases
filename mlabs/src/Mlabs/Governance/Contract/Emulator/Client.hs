...
-- imo it would be nicer if we were to take the type to be applied to 
-- callEndpoint' from the type sig itself

-- | Deposits the specified amount of GOV into the governance contract
callDeposit :: AssetClassGov -> Emulator.Wallet -> Api.Deposit
            -> EmulatorTrace ()
...
callWithdraw :: AssetClassGov -> Emulator.Wallet -> Api.Withdraw 
             -> EmulatorTrace ()
...
callProvideRewards :: AssetClassGov -> Emulator.Wallet -> Api.ProvideRewards 
                   -> EmulatorTrace ()
...
queryBalance :: AssetClassGov -> Emulator.Wallet -> Api.QueryBalance 
             -> EmulatorTrace ()
