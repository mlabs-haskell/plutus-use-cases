module Mlabs.Extract.TxRead (runCompare, readExported, WbeTx(..), parseTx) where
...
import Data.ByteArray.Encoding (Base (Base16, Base64), convertToBase, convertFromBase)
...
{- convert transaction from base64 encoded WBE response
   to base16 which cardano api can deserialize
-}

--------------------------
-- Reading WBE response --
--------------------------
newtype WbeTx = WbeTx
...
parseTx :: WbeTx -> Either Hask.String  (C.Tx C.AlonzoEra)
parseTx WbeTx{..} =
