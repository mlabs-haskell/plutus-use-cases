...
getScriptAddrUtxos :: Contract w s Text 
...
-- | Get the ChainIndexTxOut at an address.
getAddrValidUtxos :: NftAppSymbol 
  -> Contract w s Text (Map.Map TxOutRef (ChainIndexTxOut, ChainIndexTx))
getAddrValidUtxos appSymbol = Map.filter validTx <$> utxosTxOutTxAt txScrAddress
  where
    validTx (cIxTxOut, _) = elem (app'symbol appSymbol) $ 
      symbols (cIxTxOut ^. ciTxOutValue)
...
getNftDatum nftId appSymbol = do
  utxos :: [Ledger.ChainIndexTxOut] <- 
    fmap fst . Map.elems <$> getAddrValidUtxos appSymbol
  let datums :: [DatumNft] =
        utxos
  ...
  case datums of
    ...
    _ : _ -> do
      Contract.logError @Hask.String 
        "More than one suitable Datum can be found. This should never happen."
      pure Nothing

{- | Gets the Datum of a specific nftId from the Script address, and applies an
  extraction function to it.
-}
getsNftDatum :: (DatumNft -> b) -> NftId -> NftAppSymbol 
             -> Contract a s Text (Maybe b)
getsNftDatum f nftId = fmap (fmap f) . getNftDatum nftId

-- | Find NFTs at a specific Address. Will throw an error if none or many are 
-- found.
  ...
  where
    findData =
      L.filter hasCorrectNft -- filter only datums with desired NftId
        . mapMaybe readTxData -- map to Maybe 
                              --   (TxOutRef, ChainIndexTxOut, DatumNft)
        . Map.toList

    readTxData (oref, (ciTxOut, ciTx)) = 
      (oref,ciTxOut,,ciTx) <$> readDatum' ciTxOut
    ...
    hasCorrectNft (_, ciTxOut, datum, _) =
      let (cs, tn) = unAssetClass $ nftAsset datum
       in tn == nftTokenName datum -- sanity check
            && case datum of
              NodeDatum datum' ->
                -- check that Datum has correct NftId
                (info'id . node'information $ datum') == nftId 
                -- check that UTXO has single NFT in Value
                  && valueOf (ciTxOut ^. ciTxOutValue) cs tn == 1 
              HeadDatum _ -> False
...
entryToPointInfo :: (TxOutRef, (ChainIndexTxOut, ChainIndexTx)) 
                 -> GenericContract PointInfo
...
getDatumsTxsOrdered nftAS = do
  ...
  return sortedDatums
  where
    withDatum :: (TxOutRef, (ChainIndexTxOut, ChainIndexTx)) 
