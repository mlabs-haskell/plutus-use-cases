...
type BalanceCall = WbeExportTx -> ExceptT WbeError IO (WbeTx 'Balanced)
type SignCall = WbeTx 'Balanced ->  ExceptT WbeError IO (WbeTx 'Signed)
...
run = do
  ...
  runExceptT runSign >>= \case
    Left e -> putStrLn $ "Sign check failed with: " <> show e
    Right _ -> pure ()
  where
    runBalance = error "TODO" :: ExceptT WbeError IO ()
    runSign = error "TODO" :: ExceptT WbeError IO ()
...
runBalanceTest ::
  ...
  WbeExportTx ->
  ExceptT WbeError IO ()
runBalanceTest goBalance goSign  utxosGetter exportTx = do
  wbeTx <- goBalance exportTx
...
runSimple :: IO (Either WbeError ())
runSimple = runExceptT $ do
  params <-
    ExceptT $
      first DecoderError
  ...
  runBalanceTest
    (WbeClient.balance clientCfg)
    (WbeClient.sign clientCfg)
