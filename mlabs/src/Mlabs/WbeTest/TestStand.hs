...
run = runExceptT $ do
  ...
  -- will be substituted by config usage
  params <- getNetParams
  socket <- getSocket
  cfg <- getClientCfg
  let connInfo = debugConnectionInfo socket
  -- will be substituted by config usage - END
...
getTestTxs params connInfo = mapM except [tx1]
    where
      tx1 = WbeExportTx 
...
debugPassphrase :: Passphrase
debugPassphrase = Passphrase "1234567891"

debugClientCfg :: WbeClientCfg
debugClientCfg = defaultWbeClientCfg debugWalletId debugPassphrase
