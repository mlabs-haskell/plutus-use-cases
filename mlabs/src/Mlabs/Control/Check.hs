...
isNonNegative :: (Applicative m, MonadError BuiltinByteString m) => BuiltinByteString -> Integer -> m ()
...
isPositiveRational :: (Applicative m, MonadError BuiltinByteString m) => BuiltinByteString -> Rational -> m ()
isPositiveRational msg val
  | val > R.fromInteger 0 = pure ()
  | otherwise = throwError $ msg <> " should be positive"

{-# INLINEABLE isUnitRange #-}
isUnitRange :: (Applicative m, MonadError BuiltinByteString m) => BuiltinByteString -> Rational -> m ()
isUnitRange msg val
