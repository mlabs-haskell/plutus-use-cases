...
import Plutus.V1.Ledger.Scripts as Scripts (Datum (getDatum), 
...
import Mlabs.Lending.Logic.Types (LendingPool (lp'currency), 
 ...
 * user deposit has grown properly on user's internal wallet for lending pool 
   state
 ...
 * user deposit has diminished properly on user's internal wallet for lending pool 
   state
...
validate lendexId _ !ctx = case (getInState, getOutState) of
  (Just !st1, Just !st2) ->
    if hasLendexId st1 && hasLendexId st2
      then all (isValidForge st1 st2) $ Value.flattenValue $ 
        Contexts.txInfoMint info
      else traceIfFalse "Bad Lendex identifier" False
  (Just _, Nothing) -> 
    traceIfFalse "Failed to find LendingPool state in outputs" False
  (Nothing, Just _) -> 
    traceIfFalse "Failed to find LendingPool state in inputs" False
  ...
  where
    ...
    getInState = getStateForOuts 
      (Contexts.txInInfoResolved <$> Contexts.txInfoInputs info)
    ...
    isValidForge :: Input -> Input -> 
      (Value.CurrencySymbol, Value.TokenName, Integer) -> Bool
    isValidForge !st1 !st2 (cur, token, !amount) = 
      case getTokenCoin st1 st2 cur token of
    ...
    -- and user pays given amount to the lending app. We go through the list of 
    -- all signatures to see if anyone acts as a user (satisfy constraints).
    isValidMint (Input _ !st1 !stVal1) (Input _ !st2 !stVal2) 
      !coin !aCoin !amount =
      ...
      where
        ...
        checkUserDepositDiff uid =
          traceIfFalse "User deposit has not growed after Mint" $
            checkUserDepositDiffBy (\dep1 dep2 -> dep2 - dep1 == amount) 
              st1 st2 coin uid
        ...
        checkScriptPays uid =
          traceIfFalse "User has not received aCoins for Mint" $
            checkScriptContext 
              (mustPayToPubKey uid $ Value.assetClassValue aCoin 
                amount :: TxConstraints () ()) ctx

    isValidBurn (Input _lendexId1 !st1 _stVal1) 
      (Input _lendexId2 !st2 _stVal2) !coin _aCoin !amount =
      ...
      where
        ...
        checkUserDepositDiff uid =
          traceIfFalse "User deposit has not diminished after Burn" $
            checkUserDepositDiffBy (\dep1 dep2 -> dep1 - dep2 == amount) 
              st1 st2 coin uid
        ...
        checkScriptPays uid =
          traceIfFalse "User does not receive for Burn" $
            checkScriptContext 
              (mustPayToPubKey uid $ 
                Value.assetClassValue coin amount :: TxConstraints () ()) ctx

    -- check change of the user deposit for state prior to transition (st1) 
    -- and after transition (st2)
    checkUserDepositDiffBy !cond !st1 !st2 !coin uid = fromRight False $ do
      !dep1 <- getDeposit uid coin st1
      !dep2 <- getDeposit uid coin st2
      pure $ cond dep1 dep2

    getDeposit uid !coin !st = evalStateT (getsWallet (Types.UserId uid) 
      coin wallet'deposit) st
