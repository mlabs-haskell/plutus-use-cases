...
import Ledger (CurrencySymbol, PaymentPubKeyHash (..), PubKeyHash, TokenName, pubKeyHash, txId)
...
withdraw cid wallet amount =
  call cid $
    Withdraw
      [(unPaymentPubKeyHash $ mockWalletPaymentPubKeyHash wallet, amount)]
