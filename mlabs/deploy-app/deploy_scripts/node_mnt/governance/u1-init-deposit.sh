...
# First initial deposit for user1 
...
INIT_DATUM_HASH=$(cardano-cli transaction hash-script-data --script-data-file $WORKDIR/plutus_files/user1-init-datum.json)
...
cardano-cli transaction build --alonzo-era --cardano-mode ${MAGIC} --protocol-params-file $pparams \
  ...
  --tx-out "$SCRIPT_ADDR + 2000000 + $AMOUNT $GOV_POLICY_ID.GOV" \
