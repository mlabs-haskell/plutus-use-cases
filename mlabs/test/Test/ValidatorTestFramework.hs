...
import PlutusTx (IsData(toBuiltinData), BuiltinData)
import Ledger.Credential ( Credential(ScriptCredential) )
import Ledger.Value
import GHC.Show (Show)

data TestCtxIn  =  PkhCtxIn (Address,Value) | ScriptCtxIn (ByteString,Value,BuiltinData) |ThisScripCtxIn(Value, BuiltinData,BuiltinData) deriving (Show)
data TestCtxOut = PkhCtxOut (Address,Value) | ScriptCtxOut (ByteString,Value,BuiltinData) |ThisScripCtxOut(Value,BuiltinData) deriving (Show)
...
builderLockInThisScript d v=TestContextBuilder{
    ctxInputs=[],
    ctxOutputs=[ThisScripCtxOut ( v, toBuiltinData d)],
    ctxSignatures=[]
  }
...
builderLockInScript bs d v =TestContextBuilder{
    ctxInputs=[],
    ctxOutputs=[ScriptCtxOut (bs,v,toBuiltinData d)],
    ctxSignatures=[]
  }
...
builderRedeem r v d =TestContextBuilder{
    ctxInputs=[ThisScripCtxIn (v,toBuiltinData d,toBuiltinData r)],
    ctxOutputs=[],
    ctxSignatures=[]
  }
...
-- cannotBuyWithLesserValue= mkMarket defaultMarket  (toBuiltinData $ toInteger 32) Buy 
